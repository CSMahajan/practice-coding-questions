...
*/
public class CountMinimumBitFlipsToConvertAtoB {

    //TC:O(31)
    //SC:O(1)
    public int minBitFlips(int start, int goal) {
        //xor operator helps to identify unequality of bits in a number
        int result = start ^ goal;
        //xor result or given 2 numbers possess all the set bits which need to be flipped
        int count = 0;
        //below while loop is used for counting set bits in a number
        while (result > 0) {
    ...
    public static void main(String[] args) {
        CountMinimumBitFlipsToConvertAtoB cmb = new CountMinimumBitFlipsToConvertAtoB();
        System.out.println(cmb.minBitFlips(10,7));
    }
}
