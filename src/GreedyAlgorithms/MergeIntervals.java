...
public class MergeIntervals {
    ...
    public int[][] merge(int[][] intervals) {
        //sort the numbers based on start of every interval in ascending order
        ...
        for (int i = 0; i < intervals.length; i++) {
            if (intervals[i][0] <= end) {
                ...
                //create a new interval and add it to final answer
                mergedIntervals.add(new int[]{start, end});
                start = intervals[i][0];
                end = intervals[i][1];
            }
        }
        //at the last iteration gets over but final interval is not yet added, adding them now
        mergedIntervals.add(new int[]{start, end});
        return mergedIntervals.toArray(new int[0][]);
    }
